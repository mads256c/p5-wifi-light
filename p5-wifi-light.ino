...
static inline void AddDevice(const char *const mac, const char *const ip, const char *const gateway)
{
  ...
  sprintf(buffer, "GET /api/adddevice.php?name=Light&type=2&mac=%s&ip=%s HTTP/1.1\r\nHost: %s\r\n\r\n", mac, ip, gateway);
...
static inline bool GetState(const char*const mac, const char *const gateway)
{
  ...
  //sprintf(buffer, "GET /www/test.html?mac=%s HTTP/1.1\r\nHost: %s\r\n\r\n", mac, gateway);
...
void loop()
{
  ...
  digitalWrite(2, GetState(mac, gatewaya) ? HIGH : LOW);
